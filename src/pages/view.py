...
data_temp = random_timeseries(36.5, 155, 600)  # 2400
...
fig_acc = go.Figure()
fig_acc.add_trace(go.Scatter(y=data_x, mode="lines", name="x"))
fig_acc.add_trace(go.Scatter(y=data_y, mode="lines", name="y"))
fig_acc.add_trace(go.Scatter(y=data_z, mode="lines", name="z"))
fig_acc.update_layout(yaxis_title="g", margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_bvp = go.Figure()
fig_bvp.add_trace(go.Scatter(y=data_bvp, mode="lines", name="BVP"))
fig_bvp.update_layout(yaxis_title="PPG", margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_eda = go.Figure()
fig_eda.add_trace(go.Scatter(y=data_eda, mode="lines", name="EDA"))
fig_eda.update_layout(yaxis_title="uS", margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_hr = go.Figure()
fig_hr.add_trace(go.Scatter(y=data_hr_e4, mode="lines", name="E4"))
fig_hr.add_trace(go.Scatter(y=data_hr_ecg, mode="lines", name="ECG"))
fig_hr.update_layout(margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_ibi = go.Figure()
fig_ibi.add_trace(go.Scatter(y=data_ibi, mode="lines", name="IBI"))
fig_ibi.update_layout(margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_temp = go.Figure()
fig_temp.add_trace(go.Scatter(y=data_temp, mode="lines", name="Body Temperature"))
fig_temp.update_layout(yaxis_title="C", margin=dict(l=1, r=1, t=1, b=1), height=100)
...
fig_bw.update_layout(
    yaxis_title="relative power", margin=dict(l=1, r=1, t=1, b=1), height=100
)
...
fig_att_med.update_layout(margin=dict(l=1, r=1, t=1, b=1), height=100)

fig_audio = go.Figure()
fig_audio.add_trace(go.Scatter(y=data_audio, mode="lines", name="audio"))
fig_audio.update_layout(margin=dict(l=1, r=1, t=1, b=1), height=100)
#######################################################################################
###### emotion label ###################
emotion_label = [
    dict(
...
fig_temp.update_layout(shapes=emotion_label)
fig_temp.update_layout(
    {"plot_bgcolor": "rgba(0,0,0,0)", "paper_bgcolor": "rgba(0,0,0,0)"}
)
fig_att_med.update_layout(shapes=emotion_label)
    ...
    children=[
        ...
        html.Div(
            ...
            dbc.Accordion(
                [
                    dbc.AccordionItem(
                        dcc.Graph(figure=fig_acc, id="acc"), title="3-axis Acceleration"
                    ),
                    dbc.AccordionItem(
...
def update_plots(active_range):
    for fig in [fig_acc, fig_bvp]:
        fig.update_layout(xaxis=dict(range=active_range))
    return fig_acc, fig_bvp
